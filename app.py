...
<style>
:root{
--v:#14532d;--v2:#1f7a3a;--r:#b91c1c;--a:#facc15;--n:#111827;--bg:#f4f8f5;
--s:0 22px 55px rgba(17,24,39,.13)
}
...
.center{display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px;background:radial-gradient(circle at top left,rgba(250,204,21,.25),transparent 28%),radial-gradient(circle at bottom right,rgba(185,28,28,.18),transparent 28%),linear-gradient(135deg,var(--v),var(--v2))}
...
.sidebar{width:285px;background:linear-gradient(180deg,var(--v),#0f3d2e);color:white;padding:28px;border-right:8px solid var(--a);position:sticky;top:0;height:100vh}
...
.top-new{background:white;border-radius:30px;padding:28px 32px;box-shadow:var(--s);border-top:7px solid var(--a);margin-bottom:26px;display:grid;grid-template-columns:1fr auto;gap:20px;align-items:center}
.top-school{display:flex;gap:18px;align-items:center}
.top-school img{width:78px;height:78px;object-fit:contain;background:white;border-radius:18px;padding:6px;border:1px solid #e5e7eb}
.top-school h1{margin:0;color:var(--v);font-size:30px}
.top-school p{margin:5px 0;color:#4b5563}
.top-actions{text-align:right}
.badge{display:inline-block;background:#ecfdf5;color:var(--v);padding:8px 12px;border-radius:999px;font-weight:800;margin-bottom:10px}

.dashboard-grid{display:grid;grid-template-columns:1.3fr .7fr;gap:24px;margin-bottom:24px}
.institution-status{background:linear-gradient(135deg,var(--v),var(--v2));color:white;border-radius:30px;padding:34px;box-shadow:var(--s);border-bottom:8px solid var(--a)}
.institution-status h2{font-size:34px;margin:0 0 10px;color:white}
.institution-status p{font-size:16px;line-height:1.6}
.quick-panel{background:white;border-radius:30px;padding:28px;box-shadow:var(--s);border-top:7px solid var(--a)}
.quick-panel h2{color:var(--v);margin-top:0}
.quick-list{display:grid;gap:12px}
.quick-list a{background:#f8fafc;text-decoration:none;padding:15px;border-radius:16px;border-left:6px solid var(--v2)}

.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:18px;margin-bottom:24px}
.stat{background:white;border-radius:24px;padding:24px;box-shadow:var(--s);border-left:8px solid var(--v2)}
.stat h3{font-size:38px;margin:0;color:var(--v)}
.stat p{margin:6px 0 0;color:#4b5563}
.stat.red{border-left-color:var(--r)}.stat.yellow{border-left-color:var(--a)}
...
.qr-img{width:88px}.danger-link{color:var(--r)}
.portal{width:100%;max-width:460px;text-align:center}.portal #reader{max-width:330px;margin:16px auto;border-radius:18px;overflow:hidden}
...
.carnet-head h3{color:white}.carnet .qr{width:180px;margin:18px auto}
...
.footer-brand{margin-top:30px;text-align:center;color:#64748b;font-size:14px}
.footer-brand strong{color:var(--v)}

.contact-hero{text-align:center;margin-bottom:35px}
.contact-hero small{color:#0b7cff;font-weight:900;letter-spacing:1px}
.contact-hero h1{font-size:42px;margin:10px auto;max-width:820px;color:#1f2937}
.contact-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:24px}
.contact-card{background:white;border-radius:24px;padding:24px;box-shadow:var(--s);border-top:6px solid var(--a)}
.contact-icon{width:54px;height:54px;border-radius:16px;background:#0b7cff;color:white;display:flex;align-items:center;justify-content:center;font-size:24px;margin-bottom:14px}
.contact-card h3{margin:0;color:#111827}
.contact-card p{line-height:1.6;color:#334155}
.contact-card a{font-size:18px;color:#0b7cff}
.slogan-box{margin-top:30px;background:linear-gradient(135deg,var(--v),var(--v2));color:white;border-radius:28px;padding:30px;text-align:center;box-shadow:var(--s);border-bottom:8px solid var(--a)}
.slogan-box h2{color:white;margin:0}
.slogan-box p{font-size:18px}

@media(max-width:1000px){
.layout{flex-direction:column}
.sidebar{width:100%;height:auto;position:relative;border-right:0;border-bottom:8px solid var(--a)}
.dashboard-grid,.two-columns,.stats,.top-new,.contact-grid{grid-template-columns:1fr}
.main{padding:18px}
...
def footer_brand():
    ...
    <div class="footer-brand">
        <strong>{APP_NAME}</strong> © 2026 · Tecnología para el control inteligente de asistencia estudiantil<br>
    ...
    <section class="card">
        ...
        <div class="contact-grid">
            <div class="contact-card">
                ...
                <p>📞 <a href="tel:{SOPORTE_TELEFONO}">{SOPORTE_TELEFONO}</a></p>
                <p>✉️ <a href="mailto:{SOPORTE_EMAIL}">{SOPORTE_EMAIL}</a></p>
            </div>
            ...
            <div class="contact-card">
                ...
                <p>📞 <a href="tel:{CARTERA_TELEFONO}">{CARTERA_TELEFONO}</a></p>
                <p>✉️ <a href="mailto:{CARTERA_EMAIL}">{CARTERA_EMAIL}</a></p>
            </div>
        </div>

        <div class="slogan-box">
            <h2>{APP_NAME}</h2>
            <p>Tecnología para el control inteligente de asistencia estudiantil</p>
        </div>
    </section>
    """
    return page("Contacto - EduTrack QR", shell(content))
...
def dashboard():
    ...
    for g in grados:
        ...
        filas_grupo += f"""
        <tr>
            <td>{g}</td><td>{len(est_g)}</td><td>{len(ing_g)}</td>
            <td>{temp_g}</td><td>{tard_g}</td><td>{nol_g}</td>
        </tr>
        """
    ...
    filas_ultimos = "".join(
        f"<tr><td>{i.estudiante.grado}</td><td>{i.estudiante.nombre} {i.estudiante.apellido}</td><td>{i.hora}</td><td>{i.estado}</td></tr>"
        for i in ultimos
    )
    ...
    <header class="top-new">
        <div class="top-school">
            ...
            <div>
                ...
                <p>Periodo actual: <b>{PERIODO_ACTUAL}</b> · Fecha: <b>{hoy}</b></p>
            </div>
        </div>
    ...
    <section class="dashboard-grid">
        <div class="institution-status">
            ...
            <p>Vista general para coordinación, rectoría y secretaría. Aquí se resumen los ingresos de portería, novedades por grupo y asistencia tomada por docentes.</p>
            <p><b>{novedad}</b></p>
        </div>
    ...
    <section class="two-columns">
        ...
        <div class="card">
            ...
            <p>Exporta reportes institucionales con encabezado oficial del colegio.</p>
    ...
    <section class="table-card">
        ...
        <table>
            <tr><th>Grado</th><th>Matriculados</th><th>Registrados</th><th>Temprano</th><th>Tarde</th><th>No llegó</th></tr>
            {filas_grupo}
        </table>
    </section>
    ...
    <section class="table-card">
        ...
        <table>
            <tr><th>Grado</th><th>Estudiante</th><th>Hora</th><th>Estado</th></tr>
            {filas_ultimos}
        </table>
    </section>
    """
...
def portal():
    ...
    <div class="center">
        <section class="card portal">
            ...
            {mensaje_html}
            <p><a href="/login">Administración</a> · <a href="/contacto">Contacto</a></p>
            {footer_brand()}
        </section>
    </div>
...
def docente_login():
    ...
    <div class="center">
        <section class="card login-card">
            ...
            <p><a href="/login">Volver</a> · <a href="/contacto">Contacto</a></p>
            {footer_brand()}
        </section>
    </div>
    """
...
def docente():
    ...
    opciones_grupo = "".join(f'<option {"selected" if g == grupo else ""}>{g}</option>' for g in grupos)
    ...
    <section class="table-card">
        ...
        <form method="POST">
            ...
            <table>
                <tr><th>Estudiante</th><th>Estado</th><th>Observación / Excusa</th></tr>
                {filas}
            </table>
            <br>
...
def estudiantes():
    ...
    if request.method == "POST":
        ...
        if codigo:
            ...
            if e:
                ...
                db.session.add(Estudiante(codigo=codigo, nombre=nombre, apellido=apellido, grado=grado, director=director))
            db.session.commit()
    ...
    <section class="table-card">
        ...
        <table>
            <tr><th>Código</th><th>Estudiante</th><th>Grado</th><th>Director</th><th>QR</th><th>Carné</th><th>Historial</th><th>Acción</th></tr>
            {filas}
        </table>
    </section>
    """
...
def historial_estudiante(id):
    ...
    filas_ingresos = "".join(f"<tr><td>{i.fecha}</td><td>{i.hora}</td><td>{i.estado}</td><td>{i.periodo}</td></tr>" for i in ingresos)
    filas_aula = "".join(f"<tr><td>{a.fecha}</td><td>{a.hora}</td><td>{a.docente}</td><td>{a.estado}</td><td>{a.observacion}</td></tr>" for a in aula)
    filas_excusas = "".join(f"<tr><td>{x.fecha}</td><td>{x.motivo}</td><td>{x.registrado_por}</td></tr>" for x in excusas)
    ...
    <section class="table-card">
        ...
        <table><tr><th>Fecha</th><th>Hora</th><th>Estado</th><th>Periodo</th></tr>{filas_ingresos}</table>
    </section>

    <section class="table-card">
        <h2>Asistencia en aula</h2>
        <table><tr><th>Fecha</th><th>Hora</th><th>Docente</th><th>Estado</th><th>Observación</th></tr>{filas_aula}</table>
    </section>

    <section class="table-card">
        <h2>Excusas</h2>
        <table><tr><th>Fecha</th><th>Motivo</th><th>Registrado por</th></tr>{filas_excusas}</table>
    </section>
    """
...
def usuarios():
    ...
    filas = "".join(
        f"<tr><td>{u.usuario}</td><td>{u.rol}</td><td>{u.correo}</td><td>{u.grupo_docente}</td><td><a class='danger-link' href='/eliminar_usuario/{u.id}'>Eliminar</a></td></tr>"
    ...
    <section class="table-card">
        ...
        <table>
            <tr><th>Usuario</th><th>Rol</th><th>Correo</th><th>Grupo docente</th><th>Acción</th></tr>
            {filas}
        </table>
    </section>
    """
...
def encabezado_documento_texto():
    return [
        ...
        f"Fecha de impresión: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"Aplicación: {APP_NAME} | Desarrollador: {DESARROLLADOR}"
    ]
...
def reportes():
    ...
    filas = "".join(
        f"<tr><td>{i.estudiante.grado}</td><td>{i.estudiante.nombre} {i.estudiante.apellido}</td><td>{i.fecha}</td><td>{i.hora}</td><td>{i.estado}</td><td>{i.periodo}</td></tr>"
        for i in ingresos
    )
    ...
    <header class="top-new">
        <div class="top-school">
            ...
            <div>
                ...
                <p>Exportaciones con encabezado oficial.</p>
            </div>
        </div>
        <div>
            <a class="btn" href="/exportar_excel_reportes">Excel</a>
    ...
    <section class="table-card">
        ...
        <table>
            <tr><th>Grupo</th><th>Estudiante</th><th>Fecha</th><th>Hora</th><th>Estado</th><th>Periodo</th></tr>
            {filas}
        </table>
    </section>
    """
...
def excusas():
    if not requiere_login():
        return redirect("/login")
    filas = "".join(
        f"<tr><td>{x.fecha}</td><td>{x.estudiante.nombre} {x.estudiante.apellido}</td><td>{x.estudiante.grado}</td><td>{x.motivo}</td><td>{x.registrado_por}</td></tr>"
        for x in Excusa.query.order_by(Excusa.fecha.desc()).all()
    )
    content = f"""
    <header class="top-new">
        <div class="top-school"><img src="/static/img/logo-colegio.png"><div><h1>Excusas</h1><p>Registro de excusas institucionales.</p></div></div>
        <a class="btn" href="/dashboard">Volver</a>
    </header>
    <section class="table-card">
        <table><tr><th>Fecha</th><th>Estudiante</th><th>Grado</th><th>Motivo</th><th>Registró</th></tr>{filas}</table>
    </section>
    """
...
def alertas():
    ...
    content = f"""
    <header class="top-new">
        <div class="top-school"><img src="/static/img/logo-colegio.png"><div><h1>Alertas</h1><p>Novedades por llegadas tarde y ausencias.</p></div></div>
        <a class="btn" href="/dashboard">Volver</a>
    </header>
    <section class="table-card">
        <table><tr><th>Estudiante</th><th>Grado</th><th>Alerta</th></tr>{filas}</table>
    </section>
    """
...
def recuperar():
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            vencimiento = (datetime.now() + timedelta(minutes=2)).timestamp()
...
def validar_pin():
    ...
    if request.method == "POST":
        ...
        if registro:
            ...
            if datetime.now().timestamp() > vence:
...
def soporte():
    ...
    content = f"""
    <header class="top-new">
        <div class="top-school"><img src="/static/img/logo-colegio.png"><div><h1>Panel de Soporte</h1><p>Estado técnico del sistema.</p></div></div>
        <a class="btn btn-red" href="/logout">Salir</a>
    </header>
    <section class="grid">
...
def exportar_excel_reportes():
    ...
    for i in IngresoPorteria.query.join(Estudiante).all():
        ws.append([i.estudiante.codigo, i.estudiante.nombre, i.estudiante.apellido, i.estudiante.grado, i.fecha, i.hora, i.estado, i.periodo])
    b = BytesIO()
    wb.save(b)
    b.seek(0)
...
def exportar_pdf():
    ...
    for linea in encabezado_documento_texto():
        ...
        y -= 16
    y -= 15
    pdf.drawString(50, y, "REPORTE DE INGRESOS")
    y -= 25
    for i in IngresoPorteria.query.join(Estudiante).all():
        pdf.drawString(50, y, f"{i.estudiante.nombre} {i.estudiante.apellido} | Grado {i.estudiante.grado} | {i.fecha} | {i.hora} | {i.estado}")
        y -= 20
        if y < 50:
            pdf.showPage()
            y = 760
